...
st.set_page_config(
    page_title="Analog Radio System Test",
    page_icon="📄",
    layout="wide",
...
selected_profile = st.sidebar.selectbox("🔽 Choose a Profile:", list(profiles.keys()))
st.title("📄Analog Radio System Test")
# Display profile details if a valid profile is selected
if selected_profile != "Select Profile" and profiles[selected_profile]:
    st.sidebar.subheader("🔍 Profile Details:")
    for key, value in profiles[selected_profile].items():
        st.sidebar.write(f"*{key}:* {value}")

# Upload PDF file (only enabled if a profile is selected)
if selected_profile != "Select Profile":
    ...
    if uploaded_pdf:
        ...
        # Add processing button (only enabled if PDF is uploaded)
# Set up the app layout and configuration
    ...
    # Display results in a table
        ...
        st.warning("📂 Please select profile and upload a PDF file to start the process.")
